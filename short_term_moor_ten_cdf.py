...
class ShortTermMoorTenCDF(ExplicitComponent):
	...
	def setup(self):
		...
		self.add_input('max_value_moor_ten', val=0., units='N')
	...
	def compute(self, inputs, outputs):
		...
		value_extreme = inputs['max_value_moor_ten']
	...
	def compute_partials(self, inputs, partials):
		...
		partials['short_term_moor_ten_CDF', 'max_value_moor_ten'] = np.exp(-v_z_extreme * T * np.exp(-(value_extreme - mean_extreme)**2. / (2. * stddev_extreme**2.))) * v_z_extreme * T * np.exp(-(value_extreme - mean_extreme)**2. / (2. * stddev_extreme**2.)) * (value_extreme - mean_extreme) / (stddev_extreme**2.)