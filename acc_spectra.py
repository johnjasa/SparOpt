...
class AccSpectra(ExplicitComponent):
	...
	def setup(self):
		...
		self.add_output('resp_acc_pitch', val=np.zeros(N_omega), units='(rad/s**2)**2*s/rad')
		self.add_output('resp_acc_bend', val=np.zeros(N_omega), units='(m/s**2)**2*s/rad')

		self.declare_partials('*', '*')

	def compute(self, inputs, outputs):
		outputs['resp_acc_surge'] = np.abs(inputs['Re_RAO_wave_acc_surge'] + 1j * inputs['Im_RAO_wave_acc_surge'])**2. * inputs['S_wave'] + np.abs(inputs['Re_RAO_wind_acc_surge'] + 1j * inputs['Im_RAO_wind_acc_surge'])**2. * inputs['S_wind']
		outputs['resp_acc_pitch'] = np.abs(inputs['Re_RAO_wave_acc_pitch'] + 1j * inputs['Im_RAO_wave_acc_pitch'])**2. * inputs['S_wave'] + np.abs(inputs['Re_RAO_wind_acc_pitch'] + 1j * inputs['Im_RAO_wind_acc_pitch'])**2. * inputs['S_wind']
		outputs['resp_acc_bend'] = np.abs(inputs['Re_RAO_wave_acc_bend'] + 1j * inputs['Im_RAO_wave_acc_bend'])**2. * inputs['S_wave'] + np.abs(inputs['Re_RAO_wind_acc_bend'] + 1j * inputs['Im_RAO_wind_acc_bend'])**2. * inputs['S_wind']