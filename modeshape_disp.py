...
class ModeshapeDisp(ExplicitComponent):
	...
	def setup(self):
		self.add_input('eig_vector', val=np.zeros(46), units='m')
		self.add_input('z_sparnode', val=np.zeros(13), units='m')

		self.add_output('x_sparnode', val=np.zeros(13), units='m')
	...
	def compute_partials(self, inputs, partials):
		...
		partials['x_sparnode', 'eig_vector'] = np.zeros((13,46))
		partials['x_towernode', 'eig_vector'] = np.zeros((11,46))
