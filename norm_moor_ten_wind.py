...
class NormMoorTenMWind(ExplicitComponent):
	...
	def setup(self):
		...
		self.declare_partials('Re_RAO_wind_moor_ten', 'Re_RAO_wind_fairlead', rows=np.arange(N_omega), cols=np.zeros(N_omega))
		self.declare_partials('Re_RAO_wind_moor_ten', 'K_moor')
		self.declare_partials('Im_RAO_wind_moor_ten', 'Im_RAO_wind_fairlead', rows=np.arange(N_omega), cols=np.zeros(N_omega))
