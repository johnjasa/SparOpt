...
class AccSpectrumPitch(ExplicitComponent):
	...
	def compute(self, inputs, outputs):
		outputs['resp_acc_pitch'] = np.abs(inputs['Re_RAO_wave_acc_pitch'] + 1j * inputs['Im_RAO_wave_acc_pitch'])**2. * inputs['S_wave'] + np.abs(inputs['Re_RAO_wind_acc_pitch'] + 1j * inputs['Im_RAO_wind_acc_pitch'])**2. * inputs['S_wind'] + np.abs(inputs['Re_RAO_Mwind_acc_pitch'] + 1j * inputs['Im_RAO_Mwind_acc_pitch'])**2. * inputs['S_wind']