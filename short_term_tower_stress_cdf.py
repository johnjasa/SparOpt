...
class ShortTermTowerStressCDF(ExplicitComponent):
	...
	def setup(self):
		self.add_input('v_z_tower_stress', val=np.zeros(11), units='1/s')
		self.add_input('mean_tower_stress', val=np.zeros(11), units='MPa')
		self.add_input('stddev_tower_stress', val=np.zeros(11), units='MPa')
		self.add_input('max_value_tower_stress', val=np.zeros(11), units='MPa')

		self.add_output('short_term_tower_stress_CDF', val=np.zeros(11))
	...
	def compute(self, inputs, outputs):
		...
		value_extreme = inputs['max_value_tower_stress']
	...
	def compute_partials(self, inputs, partials): #TODO check
		...
		for i in xrange(11):
			...
			partials['short_term_tower_stress_CDF', 'max_value_tower_stress'][i,i] = np.exp(-v_z_extreme[i] * T * np.exp(-(value_extreme[i] - mean_extreme[i])**2. / (2. * stddev_extreme[i]**2.))) * v_z_extreme[i] * T * np.exp(-(value_extreme[i] - mean_extreme[i])**2. / (2. * stddev_extreme[i]**2.)) * (value_extreme[i] - mean_extreme[i]) / (stddev_extreme[i]**2.)