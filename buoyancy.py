...
class Buoyancy(ExplicitComponent):
	...
	def compute(self, inputs, outputs):
		...
		volume = np.pi / 4. * D_spar[0]**2. * 108. + np.pi / 4. * D_spar[-2]**2. * L_spar[-2] + np.pi / 4. * D_spar[-1]**2. * (L_spar[-1] - 10.) #TODO

		outputs['buoy_spar'] = volume * 1025. * 9.80665
		outputs['CoB'] = (np.pi / 4. * D_spar[0]**2. * 108. * (-spar_draft + 108. / 2.) + np.pi / 4. * D_spar[-2]**2. * L_spar[-2] * (-spar_draft + 108. + L_spar[-2] / 2.) + np.pi / 4. * D_spar[-1]**2. * (-spar_draft + L_spar[0] + L_spar[1] + (L_spar[-1] - 10.) / 2.)) / volume
	...
	def compute_partials(self, inputs, partials):
		...
		partials['buoy_spar', 'D_spar'] = np.array([np.pi / 2. * D_spar[0] * 108., np.pi / 2. * D_spar[-2] * L_spar[-2], np.pi / 2. * D_spar[-1] * (L_spar[-1] - 10.)]) * 1025. * 9.80665
		partials['buoy_spar', 'L_spar'] = np.array([np.pi / 4. * D_spar[0]**2., np.pi / 4. * D_spar[-2]**2., np.pi / 4. * D_spar[-1]**2.]) * 1025. * 9.80665
