...
class AccSpectrumBend(ExplicitComponent):
	...
	def compute(self, inputs, outputs):
		outputs['resp_acc_bend'] = np.abs(inputs['Re_RAO_wave_acc_bend'] + 1j * inputs['Im_RAO_wave_acc_bend'])**2. * inputs['S_wave'] + np.abs(inputs['Re_RAO_wind_acc_bend'] + 1j * inputs['Im_RAO_wind_acc_bend'])**2. * inputs['S_wind'] + np.abs(inputs['Re_RAO_Mwind_acc_bend'] + 1j * inputs['Im_RAO_Mwind_acc_bend'])**2. * inputs['S_wind']