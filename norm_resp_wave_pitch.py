...
class NormRespWavePitch(ExplicitComponent):
	...
	def setup(self):
		...
		arange = np.arange(N_omega)

		self.declare_partials('Re_RAO_wave_pitch', 'Re_wave_force_surge', rows=arange, cols=arange)
		self.declare_partials('Re_RAO_wave_pitch', 'Im_wave_force_surge', rows=arange, cols=arange)
		self.declare_partials('Re_RAO_wave_pitch', 'Re_wave_force_pitch', rows=arange, cols=arange)
		self.declare_partials('Re_RAO_wave_pitch', 'Im_wave_force_pitch', rows=arange, cols=arange)
		self.declare_partials('Re_RAO_wave_pitch', 'Re_wave_force_bend', rows=arange, cols=arange)
		self.declare_partials('Re_RAO_wave_pitch', 'Im_wave_force_bend', rows=arange, cols=arange)
		self.declare_partials('Re_RAO_wave_pitch', 'Re_H_feedbk', rows=np.repeat(arange,3), cols=Hcols)
		self.declare_partials('Re_RAO_wave_pitch', 'Im_H_feedbk', rows=np.repeat(arange,3), cols=Hcols)
		self.declare_partials('Im_RAO_wave_pitch', 'Re_wave_force_surge', rows=arange, cols=arange)
		self.declare_partials('Im_RAO_wave_pitch', 'Im_wave_force_surge', rows=arange, cols=arange)
		self.declare_partials('Im_RAO_wave_pitch', 'Re_wave_force_pitch', rows=arange, cols=arange)
		self.declare_partials('Im_RAO_wave_pitch', 'Im_wave_force_pitch', rows=arange, cols=arange)
		self.declare_partials('Im_RAO_wave_pitch', 'Re_wave_force_bend', rows=arange, cols=arange)
		self.declare_partials('Im_RAO_wave_pitch', 'Im_wave_force_bend', rows=arange, cols=arange)
		self.declare_partials('Im_RAO_wave_pitch', 'Re_H_feedbk', rows=np.repeat(arange,3), cols=Hcols)
		self.declare_partials('Im_RAO_wave_pitch', 'Im_H_feedbk', rows=np.repeat(arange,3), cols=Hcols)
