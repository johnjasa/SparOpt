...
class WindSpeed(ExplicitComponent):
	...
	def setup(self):
		...
		self.add_output('thrust_wind', val=np.zeros(3493), units='m/s')
		self.add_output('moment_wind', val=np.zeros(3493), units='m/s')
		self.add_output('torque_wind', val=np.zeros(3493), units='m/s')
