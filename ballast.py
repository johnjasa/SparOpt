...
class Ballast(ExplicitComponent):
	...
	def compute(self, inputs, outputs):
		...
		outputs['L_ball'] = (buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor_zero)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**2. * rho_ball)

		outputs['M_ball'] = (buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor_zero))
		outputs['CoG_ball'] =  -spar_draft + 0.5 * (buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor_zero)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**2. * rho_ball)
	...
	def compute_partials(self, inputs, partials):
		...
		partials['L_ball', 'D_spar'] = np.array([-2. * (buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor_zero)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**3. * rho_ball), 0., 0., 0., 0., 0., 0., 0., 0., 0.])
		partials['L_ball', 'M_turb'] = -1. / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**2. * rho_ball)
		partials['L_ball', 'M_moor_zero'] = -1. / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**2. * rho_ball)
		partials['L_ball', 'rho_ball'] = -(buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor_zero)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**2. * rho_ball**2.)
		partials['L_ball', 'wt_ball'] = 4. * (buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor_zero)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**3. * rho_ball)
		...
		partials['CoG_ball', 'D_spar'] = np.array([-(buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor_zero)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**3. * rho_ball), 0., 0., 0., 0., 0., 0., 0., 0., 0.])
		partials['CoG_ball', 'M_turb'] = -1. / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**2. * rho_ball) * 0.5
		partials['CoG_ball', 'M_moor_zero'] = -1. / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**2. * rho_ball) * 0.5
		partials['CoG_ball', 'rho_ball'] = -(buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor_zero)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**2. * rho_ball**2.) * 0.5
		partials['CoG_ball', 'wt_ball'] = 2. * (buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor_zero)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**3. * rho_ball)
