...
class Ballast(ExplicitComponent):
	...
	def compute(self, inputs, outputs):
		...
		outputs['CoG_ball'] = outputs['L_ball'] / 2. - spar_draft
	...
	def compute_partials(self, inputs, partials):
		...
		partials['L_ball', 'D_spar'] = -2. * (buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**3. * rho_ball)
		...
		partials['L_ball', 'wt_ball'] = 2. * (buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**3. * rho_ball)
		partials['L_ball', 'spar_draft'] = 0.

		partials['M_ball', 'buoy_spar'] = 1. / 9.80665
		partials['M_ball', 'tot_M_spar'] = -1.
		partials['M_ball', 'D_spar'] = 0.
		...
		partials['CoG_ball', 'D_spar'] = -(buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**3. * rho_ball)
		...
		partials['CoG_ball', 'wt_ball'] = (buoy_spar / 9.80665 - (tot_M_spar + M_turb + M_moor)) / (np.pi / 4. * (D_spar[0] - 2. * wt_ball)**3. * rho_ball)
