...
class Afeedbk(ExplicitComponent):
	...
	def setup(self):
		self.add_input('A_s_fb', val=np.zeros((7,7)))
	...
	def compute(self, inputs, outputs):
		outputs['A_feedbk'] = np.concatenate((np.concatenate((inputs['A_s_fb'],inputs['BsCc']),1),np.concatenate((inputs['BcCs'],inputs['A_contrl']),1)),0)
	...
	def compute_partials(self, inputs, partials):
		...
		partials['A_feedbk', 'A_s_fb'] = np.concatenate((As_arr2,np.zeros((18,49))),0)
