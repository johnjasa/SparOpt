...
class NormVelWindSurge(ExplicitComponent):
	...
	def setup(self):
		...
		self.declare_partials('Im_RAO_wind_vel_surge', 'Re_RAO_wind_surge', rows=np.arange(N_omega), cols=np.arange(N_omega))
		self.declare_partials('Im_RAO_wind_vel_surge', 'Im_RAO_wind_surge', rows=np.arange(N_omega), cols=np.arange(N_omega))
	...
	def compute_partials(self, inputs, partials): #TODO check
		...
		partials['Re_RAO_wind_vel_surge', 'Re_RAO_wind_surge'] = np.zeros(N_omega)
		partials['Re_RAO_wind_vel_surge', 'Im_RAO_wind_surge'] = -omega
		partials['Im_RAO_wind_vel_surge', 'Re_RAO_wind_surge'] = omega
		partials['Im_RAO_wind_vel_surge', 'Im_RAO_wind_surge'] = np.zeros(N_omega)