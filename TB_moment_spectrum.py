...
class TBMomentSpectrum(ExplicitComponent):
	...
	def setup(self):
		...
		self.add_input('Re_RAO_Mwind_TB_moment', val=np.zeros(N_omega), units='N*m/(m/s)')
		self.add_input('Im_RAO_Mwind_TB_moment', val=np.zeros(N_omega), units='N*m/(m/s)')
		...
		self.declare_partials('resp_TB_moment', 'Re_RAO_Mwind_TB_moment', rows=np.arange(N_omega), cols=np.arange(N_omega))
		self.declare_partials('resp_TB_moment', 'Im_RAO_Mwind_TB_moment', rows=np.arange(N_omega), cols=np.arange(N_omega))
	...
	def compute(self, inputs, outputs):
		outputs['resp_TB_moment'] = np.abs(inputs['Re_RAO_wave_TB_moment'] + 1j * inputs['Im_RAO_wave_TB_moment'])**2. * inputs['S_wave'] + np.abs(inputs['Re_RAO_wind_TB_moment'] + 1j * inputs['Im_RAO_wind_TB_moment'])**2. * inputs['S_wind'] + np.abs(inputs['Re_RAO_Mwind_TB_moment'] + 1j * inputs['Im_RAO_Mwind_TB_moment'])**2. * inputs['S_wind']
	...
	def compute_partials(self, inputs, partials):
		...
		partials['resp_TB_moment', 'Re_RAO_Mwind_TB_moment'] = 2. * inputs['Re_RAO_Mwind_TB_moment'] * inputs['S_wind']
		partials['resp_TB_moment', 'Im_RAO_Mwind_TB_moment'] = 2. * inputs['Im_RAO_Mwind_TB_moment'] * inputs['S_wind']
		partials['resp_TB_moment', 'S_wave'] = np.abs(inputs['Re_RAO_wave_TB_moment'] + 1j * inputs['Im_RAO_wave_TB_moment'])**2.
		partials['resp_TB_moment', 'S_wind'] = np.abs(inputs['Re_RAO_wind_TB_moment'] + 1j * inputs['Im_RAO_wind_TB_moment'])**2. + np.abs(inputs['Re_RAO_Mwind_TB_moment'] + 1j * inputs['Im_RAO_Mwind_TB_moment'])**2.