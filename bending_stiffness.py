...
class BendingStiffness(ExplicitComponent):
	...
	def compute(self, inputs, outputs):
		...
		outputs['K57'] = K_moor * z_moor * f_psi_spar(z_moor)**2. + (buoy_spar * CoB - np.sum(M_secs) * 9.80665 * CoG_spar - M_ball * 9.80665 * CoG_ball - M_moor * 9.80665 * z_moor + 1025. * 9.80665 * np.pi/64. * D_secs[0]**4.) * f_psi_d_spar(0.)
		outputs['K77'] = (buoy_spar * CoB - np.sum(M_secs) * 9.80665 * CoG_spar - M_ball * 9.80665 * CoG_ball - M_moor * 9.80665 * z_moor + 1025. * 9.80665 * np.pi/64. * D_secs[0]**4.) * f_psi_d_spar(0.)**2. + K_moor * f_psi_spar(z_moor)**2.
		...
		for i in xrange(Nelem):
			...
			for j in xrange(len(Z_tower)-1):
				if (z[i] < Z_tower[j+1]) and (z[i] >= Z_tower[j]):
					m = m_elem_tower[j]
					EI = EI_tower[j]
