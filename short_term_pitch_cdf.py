...
class ShortTermPitchCDF(ExplicitComponent):
	...
	def setup(self):
		...
		self.add_input('max_value_pitch', val=0., units='rad')
	...
	def compute(self, inputs, outputs):
		...
		value_extreme = inputs['max_value_pitch']
	...
	def compute_partials(self, inputs, partials):
		...
		partials['short_term_pitch_CDF', 'max_value_pitch'] = np.exp(-v_z_extreme * T * np.exp(-(value_extreme - mean_extreme)**2. / (2. * stddev_extreme**2.))) * v_z_extreme * T * np.exp(-(value_extreme - mean_extreme)**2. / (2. * stddev_extreme**2.)) * (value_extreme - mean_extreme) / (stddev_extreme**2.)