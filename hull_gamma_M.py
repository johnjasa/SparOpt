...
class HullGammaM(ExplicitComponent):
	...
	def setup(self):
		...
		self.add_output('gamma_M_hull', val=np.ones(10))

		self.declare_partials('gamma_M_hull', 'lambda_s', rows=np.arange(10), cols=np.arange(10))
	...
	def compute(self, inputs, outputs):
		...
		for i in xrange(len(inputs['lambda_s'])):
			...
			outputs['gamma_M_hull'][i] = gamma_M

	def compute_partials(self, inputs, partials):
		
		for i in xrange(len(inputs['lambda_s'])):
			if inputs['lambda_s'][i] >= 0.5 and inputs['lambda_s'][i] <= 1.0:
				partials['gamma_M_hull', 'lambda_s'][i] = 0.6
			else:
				partials['gamma_M_hull', 'lambda_s'][i] = 0.