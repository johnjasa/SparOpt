...
class ShortTermSurgeCDF(ExplicitComponent):
	...
	def compute(self, inputs, outputs):
		...
		value_extreme = inputs['maxval_surge']
	...
	def compute_partials(self, inputs, partials):
		...
		partials['short_term_surge_CDF', 'maxval_surge'] = np.exp(-v_z_extreme * T * np.exp(-(value_extreme - mean_extreme)**2. / (2. * stddev_extreme**2.))) * v_z_extreme * T * np.exp(-(value_extreme - mean_extreme)**2. / (2. * stddev_extreme**2.)) * (value_extreme - mean_extreme) / (stddev_extreme**2.)